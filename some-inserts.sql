...
('Verificar Disco Cheio', 'Monitora espaço em disco abaixo de 10%', 'POSTGRESQL', 'SELECT * FROM system_metrics WHERE disk_free < 10', 'MEDIUM', 120000, 3, 5000, '06:00:00', '22:00:00', true, true, false, (SELECT id FROM users WHERE email = 'admin@admin.com' LIMIT 1)),
('Verificar Conexões BD', 'Monitora conexões ativas no banco de dados', 'POSTGRESQL', 'SELECT count(*) FROM pg_stat_activity WHERE state = ''active''', 'LOW', 180000, 2, 3000, '00:00:00', '23:59:59', true, true, false, (SELECT id FROM users WHERE email = 'admin@admin.com' LIMIT 1)),
('Verificar Logs de Erro', 'Monitora logs de erro críticos', 'POSTGRESQL', 'SELECT * FROM application_logs WHERE level = ''ERROR'' AND created_at > NOW() - INTERVAL ''5 minutes''', 'HIGH', 90000, 4, 8000, '00:00:00', '23:59:59', true, false, false, (SELECT id FROM users WHERE email = 'admin@admin.com' LIMIT 1));
